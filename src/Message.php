...
use Msonowal\HL7\Exceptions\HL7Exception;
use Msonowal\HL7\Segments\MSH;
 ...
 */
class Message implements DefaultControlCharacters
{
    /**
    ...
    public function __construct(string $msgStr = null, array $hl7Globals = null, bool $keepEmptySubFields = false)
    {
        ...
        $this->segmentSeparator = $hl7Globals['SEGMENT_SEPARATOR'] ?? self::DEFAULT_SEGMENT_SEPARATOR;
        $this->segmentEndingBar = $hl7Globals['SEGMENT_ENDING_BAR'] ?? self::DEFAULT_SEGMENT_ENDING_BAR; // Bar (|) at end of each segment. Default: Present
        $this->fieldSeparator = $hl7Globals['FIELD_SEPARATOR'] ?? self::DEFAULT_FIELD_SEPARATOR;
        $this->componentSeparator = $hl7Globals['COMPONENT_SEPARATOR'] ?? self::DEFAULT_COMPONENT_SEPARATOR;
        $this->subcomponentSeparator = $hl7Globals['SUBCOMPONENT_SEPARATOR'] ?? self::DEFAULT_SUBCOMPONENT_SEPARATOR;
        $this->repetitionSeparator = $hl7Globals['REPETITION_SEPARATOR'] ?? self::DEFAULT_REPETITION_SEPARATOR;
        $this->escapeChar = $hl7Globals['ESCAPE_CHAR'] ?? self::DEFAULT_ESCAPE_CHARACTER;
        $this->hl7Version = $hl7Globals['HL7_VERSION'] ?? self::DEFAULT_HL7_VERSION;
        ...
        if ($msgStr) {
            ...
            foreach ($segments as $i => $iValue) {
                ...
                $className = "Msonowal\\HL7\\Segments\\$name";
